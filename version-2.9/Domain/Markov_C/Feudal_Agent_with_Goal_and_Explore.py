...
class Feudal_Agent_with_Goal(DomainBehavior):
    ...
    def __init__(self, agentId = '', modelFilename='', statesOfAgent={}, actions=[], isTopLevel=True, rewardMax=4, gamma=0.9, epsilon=0.1, alpha=0.1, penalty=0.1):
        ...
        # Table of Q-values
        self.optimisticReward = self.gamma*self.rewardMax - self.penalty #TODO expected best reward in a state next to the exit goal
        self.qValue = {}
        for t in self.tasks:
            self.qValue[t] = {}
            for s in self.agentStates:
                ...
                for a in self.actions[s]:
                    self.qValue[t][s][a] = self.optimisticReward # Optimistic initial conditions to make the agent want to explore all space"""
        ...
        if len(self.goals) == 0:
            ...
            with open(self.modelFilename, 'a') as f:
                f.write(json.dumps({'tasks'               : self.tasks,
                                    ...
                                    'qValue'              : self.qValue}))
        elif self.isTopLevel:
            print('nbEpisodes='+str(self.nbEpisodes))
    ...
    def selectOptimalAction(self, utility):
        ...
        for a in self.actions[self.currentState][2:]:
    ...
    def selectNextAction(self):
        ...
        if self.task == 'Explore':
            # First action is FindGoal, we don't want it
            randomAction = random.randint(1, len(self.actions[self.currentState])-1)
    ...
    def updateQvalue(self, newState, nbSteps): # for inline learning
        ...
        for t in self.tasks[2:]:
            ...
            if self.currentState == goal:
                ...
                new_qValue_S1_A1 = reward + (self.gamma**nbSteps)*max_qValue_S2_A2
