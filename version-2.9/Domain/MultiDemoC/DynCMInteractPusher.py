...
-------------------------------------------------------------------------------
 ...
 Authors:  L. Capocchi (capocchi@univ-corse.fr), J.F Santucci (santucci@univ-corse.fr), C. Kessler (kessler_c@univ-corse.fr)
 Date:     10-06-2016
-------------------------------------------------------------------------------
"""
...
from Domain.Collector import MessagesCollector, To_Pusher

import os

#    ======================================================================    #
class DynCMInteractPusher(DomainStructure):
	...
	def __init__(self,  nbMissile=10, 
		...
		### output results via Pusher
		self.push = To_Pusher.To_Pusher()
		self.push.name="MissileToPusher"
		self.push.timeNext = self.push.timeLast = self.push.myTimeAdvance = 0.
		self.addSubModel(self.push)
			
		self.push.addInPort()
		self.push.IPorts[0].name = "Missiles"
		for missile in self.missile_list:
			self.connectPorts(missile.OPorts[0],self.push.IPorts[0])
			
		self.push.addInPort()
		self.push.IPorts[1].name = "Interceptors"
		for interceptor in self.interceptor_list:
			self.connectPorts(interceptor.OPorts[0],self.push.IPorts[1])
		
		self.push.addInPort()
		self.push.IPorts[2].name = "RatioMissile"
		self.connectPorts(self.ratio.OPorts[0],self.push.IPorts[2])
		self.push.addInPort()
		self.push.IPorts[3].name = "RatioInterceptor"
		self.connectPorts(self.ratio.OPorts[1],self.push.IPorts[3])			
	...
	def modelTransition(self, state):
		...
		if 'ratio:status' in state and state['ratio:status'] == 'INIT':
			...
			self.connectPorts(m.OPorts[0],self.push.IPorts[0])
			...
			print "%i interceptor(s) added; "%(len(self.interceptor_list)-self.nbInterceptor)			
